...
export default function HelpersPage() {
  ...
  return <main className="min-h-screen bg-[#f4f9f7] text-[#183d38]">
    ...
    <section className="mx-auto max-w-[1180px] px-5 pb-20 pt-10"><p className="font-bold uppercase tracking-[.12em] text-sm text-[#e07b4c]">{t('savedCommunity')}</p><h1 className="mt-2 text-4xl font-extrabold tracking-[-.045em] sm:text-5xl">{pageTitle}</h1><p className="mt-4 max-w-2xl text-[17px] leading-7 text-[#678079]">{pageDescription}</p>
      ...
      <div className="mt-8 flex max-w-xl items-center gap-3 rounded-xl border-2 border-[#d8e7e2] bg-white px-4 focus-within:border-[#187864]"><Search size={20} className="text-[#6e8880]"/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={view === 'helpers' ? t('searchHelpers') : t('searchFinders')} className="w-full bg-transparent py-4 text-[16px] outline-none placeholder:text-[#99ada8]"/></div>
...
function HelperList({ helpers, activeProfile, onDelete, loadError }: { helpers: LocalHelper[]; activeProfile: ActiveProfile | null; onDelete: (role: 'helper' | 'requester', id: string) => void; loadError: string }) {
  const { t } = useLanguage()
  if (helpers.length) return <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{helpers.map((helper) => { const isOwn = activeProfile?.role === 'helper' && activeProfile.id === helper.id; return <article key={helper.id} className="rounded-2xl border border-[#dbe9e4] bg-white p-6 shadow-sm"><div className="flex items-start justify-between"><span className="grid h-14 w-14 place-items-center rounded-full bg-[#e7f5ef] text-lg font-extrabold text-[#187864]">{helper.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><span className="inline-flex items-center gap-1 text-xs font-bold text-[#187864]">{isOwn ? t('yourProfile') : <><HandHeart size={15}/> {t('helper')}</>}</span></div><h2 className="mt-5 text-xl font-extrabold">{helper.name}</h2><div className="mt-5 grid gap-3 text-sm text-[#59756d]"><p className="flex gap-2"><Clock3 size={18} className="shrink-0 text-[#187864]"/><span><strong className="block text-[#28554d]">{t('available')}</strong>{helper.timeAvailableForWork}</span></p><p className="flex gap-2"><Wallet size={18} className="shrink-0 text-[#187864]"/><span><strong className="block text-[#28554d]">{t('pay')}</strong>{helper.pay}</span></p><p className="flex gap-2"><MapPin size={18} className="shrink-0 text-[#187864]"/><span><strong className="block text-[#28554d]">{t('area')}</strong>{helper.address}</span></p></div>{isOwn ? <OwnProfileActions onDelete={() => onDelete('helper', helper.id)} /> : <a href={`tel:${helper.phoneNumber.replace(/\s/g, '')}`} className="mt-6 flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#187864] px-4 font-bold text-white hover:bg-[#126653]"><Phone size={18}/> {t('contact')} {helper.name.split(' ')[0]}</a>}</article> })}</div>
