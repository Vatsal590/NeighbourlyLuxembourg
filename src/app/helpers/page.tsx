...
function HelperList({ helpers, activeProfile, onDelete, loadError }: { helpers: LocalHelper[]; activeProfile: ActiveProfile | null; onDelete: (role: 'helper' | 'requester', id: string) => void; loadError: string }) {
  const { t } = useLanguage()
  if (helpers.length) return <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{helpers.map((helper) => { const isOwn = activeProfile?.role === 'helper' && activeProfile.id === helper.id; return <article key={helper.id} className="rounded-2xl border border-[#dbe9e4] bg-white p-6 shadow-sm"><div className="flex items-start justify-between"><span className="grid h-14 w-14 place-items-center rounded-full bg-[#e7f5ef] text-lg font-extrabold text-[#187864]">{helper.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><span className="inline-flex items-center gap-1 text-xs font-bold text-[#187864]">{isOwn ? t('yourProfile') : <><HandHeart size={15}/> {helper.verificationStatus === 'identity-checked' ? t('identityChecked') : t('communityHelper')}</>}</span></div><h2 className="mt-5 text-xl font-extrabold">{helper.name}</h2><p className="mt-2 text-sm font-semibold text-[#58746d]">{helper.services?.join(' · ') || t('everyday')}</p><div className="mt-5 grid gap-3 text-sm text-[#59756d]"><p className="flex gap-2"><Clock3 size={18} className="shrink-0 text-[#187864]"/><span><strong className="block text-[#28554d]">{t('available')}</strong>{helper.timeAvailableForWork}</span></p><p className="flex gap-2"><Wallet size={18} className="shrink-0 text-[#187864]"/><span><strong className="block text-[#28554d]">{t('pay')}</strong>{helper.pay}</span></p><p className="flex gap-2"><MapPin size={18} className="shrink-0 text-[#187864]"/><span><strong className="block text-[#28554d]">{t('area')}</strong>{helper.address}</span></p></div>{isOwn ? <OwnProfileActions onDelete={() => onDelete('helper', helper.id)} /> : <div className="mt-6 grid gap-2"><Link href={`/helpers/${helper.id}`} className="flex min-h-12 items-center justify-center rounded-xl border-2 border-[#c9ddd7] px-4 font-bold text-[#28554d]">{t('viewHelper')}</Link><Link href={`/book/${helper.id}`} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#187864] px-4 font-bold text-white hover:bg-[#126653]">{t('requestVisit')} <ArrowRight size={18}/></Link></div>}</article> })}</div>
